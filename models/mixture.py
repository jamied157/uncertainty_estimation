...
class MixtureModel(nn.Module):
    ...
    def log_likelihood(self, x_norm, y_norm):
        ...
        if mixture_var < 0.9:
            ...
            logging.debug(
                "Combined NLL: {:.3f} or {:.3f}".format(-float(output.mean()), -float(old_output))
            )
...
class StableMixtureModel(nn.Module):
    ...
    def log_likelihood(self, x_norm, y_norm):
        ...
        output = (
            torch.log(
                (1 - only_gamma_bool) * mixture_var * torch.exp(norm_dist.log_prob(y_norm))
                + ((1 - only_normal_bool) * (1 - mixture_var) * torch.exp(gamma_dist.log_prob(y)))
            )
        ).mean()
        logging.debug("Mixture var: {}".format(float(mixture_var.mean())))
        logging.debug(
